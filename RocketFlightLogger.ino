...
void recordAltitude()
{
  ...
  //boolean apogeeHasFired[4] = {false, false, false, false};
  //boolean mainHasFired[4] = {false, false, false, false};
  ...
  while (!exitRecording)
  {
    ...
    while (liftOff)
    {
      ...
      if (!allTimerFiredComplete) {
        //SerialCom.println(F("Entering timer"));
        //fire all timers that are ready
        for (int ti = 0; ti < 4; ti++ ) {
          if (!outputHasFired[ti] && ((currentTime >= OutputDelay[ti]) ) && OutputType[ti] == 2) {
            SerialCom.print(F("timer high:"));
            SerialCom.println(ti);
...
void resetFlight() {
  ...
  /*for (int i = 0; i< 4 ; i++) {
    apogeeHasFired[i] = false;
    mainHasFired[i] = false;
    }*/
  //apogeeHasFired[] = {false,false, false, false };
  //mainHasFired[] = { false, false, false, false };
  //ResetGlobalVar();
  allApogeeFiredComplete  = false;
  allMainFiredComplete = false;
  allTimerFiredComplete = false;
